...
def get_encryption(cell):
    ...
    if matching_line(cell,"Encryption key:") == "off":
        ...
        for line in cell:
            ...
            if matching!=None:
#                wpa2=match(matching,"IEEE 802.11i/WPA2")
#		if wpa2:
#		    wpa2v=match(wpa2,"Version ")
#		    enc="WPA2 v."+wpa2v
#		else:
...
def run_iwlist_scanning(iface='wlan0', test_mode=False):
    """Pretty prints the output of iwlist scan into a table"""
    cells=[[]]
    parsed_cells=[]
    import subprocess
    #with open('iwlist_sample.txt') as fh:
    for line in subprocess.check_output(['/sbin/iwlist',iface,'scanning']).split("\n"):
    #for line in open('iwlist_sample.txt'):
	cell_line = match(line,"Cell ")
	if cell_line != None:
	    cells.append([])
	    line = cell_line[-27:]
    ...
    sort_cells(parsed_cells)
    return return_cells(parsed_cells)
#    print_cells(parsed_cells)

if __name__ == "__main__":
    print run_iwlist_scanning()
